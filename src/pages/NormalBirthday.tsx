...
const NormalBirthday = () => {
  ...
  useEffect(() => {
    ...
    // Generate confetti
    const newConfetti: Particle[] = Array.from({ length: 60 }, (_, i) => ({
      id: i,
      x: Math.random() * 100,
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-void-1">
      ...
      >
        ...
        {/* Title with glow */}
        <h1
          className="text-4xl md:text-7xl font-light mb-6 tracking-wide"
          style={{
            color: 'hsl(210 33% 97%)',
            textShadow: `
        ...
        {/* Name with emphasis */}
        <p
          className="text-3xl md:text-5xl font-extralight mb-10 tracking-widest"
          style={{ 
            color: 'hsl(43 60% 52%)',
        ...
        <div 
          ...
          style={{ animation: 'fade-in-up 1s 0.8s both' }}
        >
          {['🎈', '✨', '🎉', '💫', '🎊'].map((emoji, i) => (
            <span 
              key={i}
              ...
              style={{ 
                animation: `emoji-bounce 2s ease-in-out ${i * 0.2}s infinite`,
              }}
            >
              {emoji}
            </span>
        ...
        {/* Message */}
        <p
          className="text-lg md:text-xl font-light mb-12 max-w-md mx-auto leading-relaxed"
          style={{ 
            color: 'hsl(210 33% 80%)',
        ...
        <button
          onClick={() => navigate('/')}
          className="group relative px-8 py-4 rounded-full font-light tracking-wide transition-all duration-500 hover:scale-105"
          style={{
        ...
        >
          <span className="relative z-10">← Return to Experience</span>
          <div 
